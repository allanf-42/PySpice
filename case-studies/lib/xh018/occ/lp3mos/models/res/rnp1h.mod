...
*                        PARAMETERIZED
* ----------------------------------------------------------------------
* NOTE: High-ohmic N-doped POLY1 resistor
* NOTE: TNOM = 27 deg C
* VARIABLES: l,w=device length and width, pm=number of contacts
*
.subckt rnp1h a f l=10e-6 w=10e-6 pm=-1 par1=1 
xocc_m1 a f  rnp1h_ahdl tstep_occ=tstep_soac
.param pmc='1+int((w*1e6-0.199)/0.47)'
+ pmi='(pm>=0)?((pm==0)?1000:pm):pmc'
+ mf='1/(1+AR_rnp1h/sqrt(1e12*par1*w*l))'
r11 a b rctp1  m='mf*pmi'
r12 b c rsilnp1 m=mf w=w l=0.22e-6
xr1 c d rnpoly1h m=mf w=w l=l
